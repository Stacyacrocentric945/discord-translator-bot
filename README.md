...
- Authors : uaaw      — https://github.com/uaaw
...
Upload audio files and the bot will transcribe and translate them automatically.
...
Each user can set their own target language. Preferences are saved across restarts.
...
Respects Gemini API rate limits using a sliding window algorithm.

### Safe Embed Output
All translated content has @everyone and @here tokens escaped automatically.
...
Direct invite link (replace CLIENT_ID with your bot application ID):
```
...
Note: The free tier includes generous daily limits. The bot automatically
rotates through 6 Gemini models to maximise free-tier uptime.
...
| MAX_FALLBACK_WAIT_SECS  | 8       | Maximum seconds spent on model fallback retries  |
...
Show the full help menu with all commands and usage examples.
...
Display information about the bot's creators with their GitHub links.
...
- Reply with a formatted embed showing the original text and translation
...
3. The bot will:
   ...
   - Show both the original transcript and the translation
...
- You are replying to a message, not sending a new standalone message
...
!resetlang     Clear your preference
!lang en       Set a new target language
```
